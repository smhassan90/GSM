...
public class Util {
   WebserviceResponse responseListener;
    ...
    public void signUp(final Activity context, User user, WebserviceResponse webserviceResponse){
        this.responseListener = webserviceResponse;
        SharedPreferences editor = context.getSharedPreferences(Codes.PREF_NAME, Context.MODE_PRIVATE);
        RequestParams rp = new RequestParams();
        rp.add("userString",new Gson().toJson(user));

        JSONObject response = null;

        HttpUtils.get("insertUser", rp, new JsonHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                String message = "";
                String data = "";
                ...
                try {
                    ...
                    if(!response.isNull("user")){
                        data =  response.get("user").toString();
                    }
                }catch(Exception e){
                    Toast.makeText(context,"Something went wrong while sync",Toast.LENGTH_SHORT).show();
                }
                if(Codes.ALL_OK.equals(codeReceived)){
                    ...
                    if(user!=null && user.getId()!=null){
                        SharedPreferences.Editor editor =  context.getSharedPreferences(Codes.PREF_NAME, MODE_PRIVATE).edit();
                        editor.putString("userId", user.getId());
                        editor.apply();
                    }
                }
                responseListener.responseAlert(codeReceived);
            }
